...
from utils.helpers import save_json_file, load_json
            ...
            for attachment in bug_data_package['attachments']:
                ...
                if file_path and Path(file_path).exists():
                    ...
                    if attachment['type'] == 'pdf':
                        ...
                        # PDF processing functionality would be imported here if needed
                        # This is a placeholder for now as we're refactoring
...
"""
    ...
    def _call_gemini_api(self, user_prompt: str, raw_data_package: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Make API call with the working approach
            ...
            if hasattr(response, 'text'):
                try:
                    task_graph = json.loads(response.text)
                    self.logger.info("Successfully parsed JSON response")
                except json.JSONDecodeError:
                    self.logger.warning("Failed to parse direct JSON response, trying parts")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="gemini-2.5-pro-exp-03-25", help="Gemini model for the generator.")
